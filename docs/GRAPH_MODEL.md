...
- 表示や探索は `relation_type` / `strength` でフィルタして行う。
- 穴（抜けている前提）は、期待前提と実際の `relations` の差分をクエリで検出する。専用テーブルは設けない。
...
## PostgreSQL でのテーブル構成
- `nodes`: 技術ノードの保持。`level` は 0〜5 の整数で管理。
- `relations`: ノード同士を接続し、`relation_type` / `strength` を保持。`strength` は 0.0〜1.0 の CHECK 制約を付与。
